function output = WLSFilter(input, lambda, alpha)
...
% disp('Compute affinities between adjacent pixels based on gradients of L 1')
dy = diff(input, 1, 1);
dy = -lambda./(abs(dy).^alpha + small_eps);
dy = padarray(dy, [1 0], 'post');
dy = dy(:);

% disp('Compute affinities between adjacent pixels based on gradients of L 2')
dx = diff(input, 1, 2);
dx = -lambda./(abs(dx).^alpha + small_eps);
dx = padarray(dx, [0 1], 'post');
dx = dx(:);

% disp('Construct a five-point spatially inhomogeneous Laplacian matrix')
B(:,1) = dx;
B(:,2) = dy;
d = [-r,-1];
A = spdiags(B,d,k,k);
